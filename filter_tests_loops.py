...
h5_file_8d = data_directory + '/' + insar_frame + '/' + insar_frame + '_10d_cum_filt.h5'
...
vel_8d = data_directory + '/' + insar_frame + '/' + insar_frame + '_10d.vel.filt.geo.tif'
...
vel_nofilt = data_directory + '/' + insar_frame + '/' + insar_frame + '_nofilt.vel.geo.tif'
h5_file_nofilt = data_directory + '/' + insar_frame + '/' + insar_frame + '_nofilt.cum.h5'
...
cum_files = [h5_file_nofilt, h5_file_05k, h5_file_2k, h5_file_16k, h5_file_8d, h5_file_30d, h5_file_128d]
vel_files = [vel_nofilt, vel_05k, vel_2k, vel_16k, vel_8d, vel_30d, vel_128d]
#%%
...
titles_vels = ["No filter", "0.5km", "2km", "16km", "~8 days", "~30 days", "~128 days"]
...
plt.plot(gnss_dates_subset, GPS_dLOS, color='blue', marker="o", label='N', linestyle='None', markersize=6,
         linewidth=0.5)
ax1.set_title("GNSS LoS Displacement ({})".format(gnss_station), fontsize=16)

# Fix the loop for subplots
for i, name in enumerate(cum_subset_ts_dict):
    ax_i = plt.subplot(8, 1, i + 2)  # Use 'polar=True' for each subplot
    trans = mtransforms.ScaledTranslation(10 / 72, -5 / 72, fig.dpi_scale_trans)
    plt.plot(dates_subset_dict[name], cum_subset_ts_dict[name], color='blue', marker="o", label='N',
             linestyle='None', markersize=6, linewidth=0.5)
    ax_i.set_title("{} ({}) InSAR LoS Displacement".format(name, insar_frame), fontsize=16)

plt.xlabel('Dates', fontsize=18, fontweight='bold')
plt.ylabel('mm', fontsize=18, x=-5)
fig.suptitle("{} displacement time-series at point {}, {} ({})".format(insar_frame, gnss_lon, gnss_lat, gnss_station), fontweight='bold', fontsize=18,
             y=0.99)

plt.tight_layout()
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/time_series_{}_{}.png'.format(gnss_station, insar_frame), dpi=400, bbox_inches='tight')
plt.show()
...
# Remove the unnecessary subplot creation using fig.add_subplot(111, frame_on=False)
plt.tick_params(labelcolor="none", bottom=False, left=False)

# Fix the loop for subplots
for i, name in enumerate(cum_subset_ts_dict):
    ax_i = plt.subplot(7, 1, i+1)  # Use 'polar=True' for each subplot
    trans = mtransforms.ScaledTranslation(10 / 72, -5 / 72, fig.dpi_scale_trans)
    plt.plot(dates_subset_dict[name], cum_subset_ts_dict[name], color='blue', marker="o", label='N',
             linestyle='None', markersize=6, linewidth=0.5)
    ax_i.set_title("{} ({}) InSAR LoS Displacement".format(name, insar_frame), fontsize=16)
    # Calculate halfway point of date range for each subplot
    date_range_i = dates_subset_dict[name][-1] - dates_subset_dict[name][0]
    half_date_range_i = date_range_i / 2

    # Set x-axis limits to half of the date range for each subplot
    plt.xlim(dates_subset_dict[name][0], dates_subset_dict[name][0] + half_date_range_i)
    plt.ylim(-800, cum_subset_ts_dict[name][0])
...
plt.tight_layout()
#plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/time_series_zoom.png', dpi=400, bbox_inches='tight')
plt.show()
...
# Fix the loop for plotting all time-series on the same plot
for i, name in enumerate(cum_subset_ts_dict):
    plt.plot(dates_subset_dict[name], cum_subset_ts_dict[name], marker="*", label=name,
             linestyle='-', markersize=3, linewidth=0.5)

plt.xlabel('Dates', fontsize=14)
plt.ylabel('mm', fontsize=14, x=-5)
plt.title("{} displacement time-series at point {}, {} ({})".format(insar_frame, gnss_lon, gnss_lat, gnss_station), fontweight='bold', fontsize=14)

plt.legend(fontsize=14)
plt.tight_layout()
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/time_series_compare_{}_{}.png'.format(gnss_station, insar_frame), dpi=400, bbox_inches='tight')
plt.show()
...
for i, name in enumerate(cum_subset_ts_dict):
    ax_i = plt.subplot(8, 1, i + 2)  # Use 'polar=True' for each subplot
    plt.plot(dates_subset_dict[name], cum_subset_ts_dict[name], color='blue', marker="o", label='N', linestyle='None', markersize=6, linewidth=0.5)
    plt.plot(dates_subset_dict[name], regression_line_dict[name], 'g')
    ax_i.set_title("{} ({}) InSAR LoS Displacement".format(name, insar_frame), fontsize=16)

plt.xlabel('Dates', fontsize=18, fontweight='bold')
plt.ylabel('mm', fontsize=18, x=-5)
fig.suptitle("Displacement time-series at point {}, {}".format(gnss_lon, gnss_lat), fontweight='bold', fontsize=18,
             y=0.99)

plt.tight_layout()
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/time_series_lin_fit_{}.png'.format(gnss_station), dpi=400, bbox_inches='tight')
plt.show()
...
colours = ['peru', 'mediumblue', 'teal', 'cornflowerblue', 'fuchsia', 'hotpink', 'indigo']
...
plt.title("{} LoS displacement time-series at {} ({}, {})".format(insar_frame, gnss_station, gnss_lon,gnss_lat), fontweight='bold', fontsize = 17)

plt.legend(loc='lower left', fontsize=16)

plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/time_series_lin_fit_zeroed_{}.png'.format(gnss_station), dpi=400, bbox_inches='tight')
...
for i, name in enumerate(lats_dict):
    ...
    lats_temp = []
    for lat in lats:
        lats_temp.append(np.abs(lat_def - lat).argmin())
...
for i, name in enumerate(lons_dict):
    ...
    lons_temp = []
    for lon in lons:
        lons_temp.append(np.abs(lon_def - lon).argmin())
...
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/{}_{}_lat_profile_MMX1_ICMX.png'.format(insar_frame, ml_clip), dpi=400, bbox_inches='tight')

#%% Plot zooms onto noise and signal

plt.figure(figsize=(6,10))

# Add a line at 0 mm/yr
...
# limit plot to focus on noise
plt.xlim(-25,25)
plt.ylim(19.52, 19.64)

plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/lat_profile_noise_zoom.png', dpi=400, bbox_inches='tight')
...
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/lat_profile_signal_zoom.png', dpi=400, bbox_inches='tight')
...
# limit plot to focus on noise
plt.xlim(-20, 10)
plt.ylim(19.13, 19.27)

plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/lat_profile_zero_zoom.png', dpi=400, bbox_inches='tight')
...
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/{}_{}_lon_profile.png'.format(insar_frame, ml_clip), dpi=400, bbox_inches='tight')
plt.show()

#%%
# Plot the 'vel' variable with latitude and longitude on the axes
plt.imshow(vel_dict['No filter'], extent=[lon_min_def, lon_max_def, lat_min_def, lat_max_def], cmap = cm.vik, vmin=-200, vmax=200)
...
plt.savefig('/nfs/see-fs-02_users/eejap/public_html/filter_tests/{}_{}_insar_los_map.png'.format(insar_frame,ml_clip), dpi=400, bbox_inches='tight')
plt.show()
