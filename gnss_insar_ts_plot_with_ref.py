...
#
# This script includes referencing i.e. GNSS of interest is ref'd to another in a stable region and
# the InSAR is referenced to itself at that point too.
#
...
ref_stations = ["MXTX", "HGTO", "MXAJ"]
frames = '078A_07049_131313', '041D_07134_222019','143D_07197_212120'
gnss_ref_station_dict = dict(zip(frames, ref_stations))
gnss_ref_station = gnss_ref_station_dict[insar_frame]

filename = directorio + '/data/gnss/' + gnss_station + '.NA.tenv3'
reference_gnss = directorio + '/data/gnss/' + gnss_ref_station + '.NA.tenv3'
...
#%% Sort GNSS station of interest into good df format
...
#%% Sort ref GNSS into good df format
dfgnss_ref = pd.read_csv(reference_gnss, delimiter=r"\s+")
ref_gnss_lat = dfgnss_info.at[gnss_ref_station, 'lat']
ref_gnss_lon = dfgnss_info.at[gnss_ref_station, 'lon']
ref_start_date = dfgnss_info.at[gnss_ref_station, 'start_date'].astype(str)
ref_end_date = dfgnss_info.at[gnss_ref_station, 'end_date'].astype(str)
...
dfdata_ref = pd.DataFrame({
    'Column2': dfgnss_ref.iloc[:, 2],
    'Column8': dfgnss_ref.iloc[:, 8],
    'Column10': dfgnss_ref.iloc[:, 10],
    'Column12': dfgnss_ref.iloc[:, 12],
    'Column14': dfgnss_ref.iloc[:, 14],
    'Column15': dfgnss_ref.iloc[:, 15],
    'Column16': dfgnss_ref.iloc[:, 16]
})
#%% sort out interest gnss further
...
#%%sort out ref gnss further
dfdata_ref_array = np.squeeze(np.array(dfdata_ref))
mean_value = np.mean(dfdata_ref_array, axis=0).squeeze().reshape(1,-1)
binary_array = np.array([0, 1, 1, 1, 0, 0, 0])
scaling_array = np.array([1, 1000, 1000, 1000, 1000, 1000, 1000])

mean_value = np.tile(mean_value, (dfdata_ref_array.shape[0], 1))
binary_array = np.tile(binary_array, (dfdata_ref_array.shape[0], 1))
scaling_array = np.tile(scaling_array, (dfdata_ref_array.shape[0], 1))

dfdata_ref_array = scaling_array * (dfdata_ref_array - binary_array * mean_value)
ndaysr = len(dfdata_ref_array)
dater = dfdata_ref_array[:, 0]
Er = dfdata_array[:, 1]
Nr = dfdata_array[:, 2]
Ur = dfdata_array[:, 3]
eEr = dfdata_array[:, 4]
...
column_names = ['Dates', 'dN', 'dE', 'dU', 'Sn', 'Se', 'Su']
dfGPS_ref=pd.DataFrame(dfdata_ref_array, columns=column_names)

#%% Import reunwrapped cum files
ruw_list = ['default', 'goldstein_cascade'] #'gaussian', 'lowpass', 
tif_end = '.cum.h5'
...
titles_vels = ["No reunwrapping", "Goldstein + Cascade"] #"Gaussian", "Lowpass", 
...
#%% Find the indices of the nearest grid cell to the GNSS site of interest
...
# for referencing
lat_index_rw_ref = np.abs(lat_rw - ref_gnss_lat).argmin()
lon_index_rw_ref = np.abs(lon_rw - ref_gnss_lon).argmin()

lat_index_def_ref = np.abs(lat_def - ref_gnss_lat).argmin()
lon_index_def_ref = np.abs(lon_def - ref_gnss_lon).argmin()
...
for i, name in enumerate(cum_dict):
    cums = cum_dict[name]
    if name == 'No reunwrapping':
        cum_ts = cums[:, lat_index_def_ref, lon_index_def_ref] # if using cum file
    else:
        cum_ts = cums[:, lat_index_rw_ref, lon_index_rw_ref] # if using cum file
    cum_ts_ref_dict[name] = cum_ts # dictionary holds time-series displacement data for point at closes point to GNSS location
    

#%% better way
dfGPS['Dates'] = pd.to_datetime(dfGPS['Dates'], format='%Y') + pd.to_timedelta((dfGPS['Dates'] % 1) * 365, unit='D')
#%%
dfGPS_ref['Dates'] = pd.to_datetime(dfGPS_ref['Dates'], format='%Y') + pd.to_timedelta((dfGPS_ref['Dates'] % 1) * 365, unit='D')
...
# same for reference
cum_subset_ts_ref_dict = {}
dates_subset_ref_dict = {}
dates_dec_subset_ref_dict = {}
for i, name in enumerate(cum_ts_ref_dict):
    cums = cum_ts_ref_dict[name]
    dates_dec = dates_dec_dict[name]
    dates = dates_dict[name]
    if name == 'No reunwrapping':
        cum_subset = cums[start_index_def:end_index_def+1]
        ...
        dates_subset = dates_rw[start_index_rw:end_index_rw+1]
        dates_dec_subset = dates_dec[start_index_rw:end_index_rw+1]
    cum_subset_ts_ref_dict[name] = cum_subset
    dates_subset_ref_dict[name] = dates_subset
    dates_dec_subset_ref_dict[name] = dates_dec_subset
...
# difference main and ref time-series
for i, name in enumerate(dates_subset_dict):
    cum_plot = cum_subset_ts_dict[name]
    cum_ref_plot = cum_subset_ts_ref_dict[name]
    diff_cum = cum_plot - cum_ref_plot
    diff_cum_dict[name] = diff_cum
...
for i, name in enumerate(dates_subset_dict):
    ...
    cum_plot = cum_subset_ts_dict[name]
    plt.plot(dates_plot, cum_plot, label = '{}'.format(name))

# plot ref subset too
for i, name in enumerate(dates_subset_ref_dict):
    dates_ref_plot = dates_subset_ref_dict[name]
    cum_ref_plot = cum_subset_ts_ref_dict[name]
    plt.plot(dates_ref_plot, cum_ref_plot, label = 'Ref {}'.format(name))
    
# plot diff
for i, name in enumerate(dates_subset_ref_dict):
    dates_ref_plot = dates_subset_ref_dict[name]
    diff_cum_plot = diff_cum_dict[name]
    plt.plot(dates_ref_plot, diff_cum_plot, label = 'Refd {}'.format(name))
...
#%%
## Calculate incidence angle at the GNSS site of interest
...
## Calculate incidence angle at the GNSS ref site
inc_agl_ref = inc_agl_deg[lat_index_def_ref, lon_index_def_ref]
#%%
### Change to LOS
## Take heading from LiCS Portal metadata.txt for frame of interest
headings = -10.804014, -169.14888, -169.11923
head_dict = dict(zip(frames, headings))
 
inc=inc_agl*(np.pi/180)
inc_ref = inc_agl_ref*(np.pi/180)
head=head_dict[insar_frame]
...
#%% try to reference GNSS and InSAR to reference point before comparing vels
merged_df = dfGPS_ref.merge(dfGPS, how = 'inner', on = ['Dates'])
common_columns = ['dN', 'dE', 'dU', 'Sn', 'Se', 'Su']

# convert 2 stations data to LoS
head=head_dict[insar_frame]
az=(360+head)*(np.pi/180)
sininc=math.sin(inc_ref)
cosinc=math.cos(inc_ref)
sinaz=math.sin(az)
cosaz=math.cos(az)
dfGPS_ref_LOS = (((merged_df['dN_x']*sinaz)-(merged_df['dE_x']*cosaz))*sininc)+(merged_df['dU_x']*cosinc)
...
sinaz=math.sin(az)
cosaz=math.cos(az)
dfGPS_LOS_plot = (((merged_df['dN_y']*sinaz)-(merged_df['dE_y']*cosaz))*sininc)+(merged_df['dU_y']*cosinc)

# Rename the columns in the differences DataFrame
differences_LOS = dfGPS_LOS_plot - dfGPS_ref_LOS
#%%
# Assuming you have the diff_cum_dict containing four arrays (arrays1, arrays2, arrays3, and arrays4)
# Find the shortest array length among the arrays in diff_cum_dict
min_length = min(len(arr) for arr in diff_cum_dict.values())

# Create a new DataFrame with 'Dates' as the first column
diff_cum_dates = pd.DataFrame({'Dates': dates_subset_ref_dict['Goldstein + Cascade']})

# Truncate the longer array to match the length of the shortest array
for key, arr in diff_cum_dict.items():
    if len(arr) > min_length:
        diff_cum_dict[key] = arr[:min_length]

diff_cum_dict_df = pd.DataFrame(diff_cum_dict)

# Convert diff_cum_dict arrays to a DataFrame
diff_cum_df = pd.concat([diff_cum_dates, diff_cum_dict_df], axis=1)

# Set 'Dates' column as the DataFrame index to facilitate merging
diff_cum_df.set_index('Dates', inplace=True)

# Convert the 'Dates' index back to a regular column
diff_cum_df.reset_index(inplace=True)
#%%
# Filter the rows in diff_cum_df to match the start and end dates of merged_df
# Find the latest start date from both DataFrames
start_date = max(merged_df['Dates'].iloc[0], diff_cum_df['Dates'].iloc[0])

# Find the earliest end date from both DataFrames
end_date = min(merged_df['Dates'].iloc[-1], diff_cum_df['Dates'].iloc[-1])
diff_cum_df = diff_cum_df[(diff_cum_df['Dates'] >= start_date) & (diff_cum_df['Dates'] <= end_date)]
merged_df = merged_df[(merged_df['Dates'] >= start_date) & (merged_df['Dates'] <= end_date)]

# Reset the index of diff_cum_df to ensure proper concatenation
diff_cum_df.reset_index(drop=True, inplace=True)

# Reset the index of merged_df to ensure proper concatenation
merged_df.reset_index(drop=True, inplace=True)

diff_cum_dict = diff_cum_df.to_dict(orient='list')

#%%
## Calculate linear regression for LoS GNSS (referenced)

dates_dec_dict_ref = {}
dates = merged_df['Dates']
dates_temp = []
...
for d in dates:
    ...
    date_dec = year + fraction
    dates_temp.append(date_dec)
dates_dec_ref = np.array(dates_temp)

#merged_df['Dates_ordinal'] = merged_df['Dates'].apply(lambda date: date.toordinal())
sloperef, interceptref, r_valueref, p_valueref, std_errref = linregress(dates_dec_ref, differences_LOS[0:len(merged_df['Dates'])])
slope_plotref = format(sloperef, '.2f')
std_err_plotref = format(std_errref, '.2f')
regression_lineref = sloperef * dates_dec_ref + interceptref

# slope2, intercept2, r_value2, p_value2, std_err2 = linregress(merged_df['Dates_ordinal'], dfGPS_LOS_plot[0:len(merged_df['Dates'])])
# slope_plot2 = format(slope2, '.2f')
# std_err_plot2 = format(std_err2, '.2f')
# regression_line2= slope2 *merged_df['Dates_ordinal'] + intercept2
#%%
## Calculate linear regression for LoS InSAR

# insar_dates = diff_cum_dict['Dates']
# dates_temp = []
# for date in insar_dates:
#     date_str = str(date) 
#     date_obj = datetime.strptime(date_str, "%Y%m%d")
#     dates_temp.append(date_obj)
# insar_dates_format = np.array(dates_temp)

dates_dec_insar_ref = {}
dates_temp = []
dates = diff_cum_dict['Dates']
for d in dates:
    year = d.year
    startOfThisYear = datetime(year=year, month=1, day=1)
    startOfNextYear = datetime(year=year+1, month=1, day=1)
    ...
    date_dec = year + fraction
    dates_temp.append(date_dec)
dates_dec_insar_ref = np.array(dates_temp)

slope_dict_ref = {}
slope_plot_dict_ref = {}
std_err_dict_ref = {}
std_err_plot_dict_ref = {}
regression_line_dict_ref = {}
intercept_dict_ref = {}

for i, name in enumerate(lons_dict):
    cum_use = np.array(diff_cum_dict[name])
    dates_dec_use = diff_cum_dict[name]
    dates_subset = diff_cum_df['Dates']
    dates_dec_subset = dates_dec_insar_ref
    valid_indices = np.logical_and(~np.isnan(dates_dec_subset), ~np.isnan(cum_use))
    dates_dec_subset = dates_dec_subset[valid_indices]
    cum_use = cum_use[valid_indices]
    slope_i, intercept_i, r_value_i, p_value_i, std_err_i = linregress(dates_dec_subset, cum_use)
    slope_dict_ref[name] = slope_i
    std_err_dict_ref[name] = std_err_i
    intercept_dict_ref[name] = intercept_i
    slope_plot_dict_ref[name] = format(slope_i, '.2f')
    std_err_plot_dict_ref[name] = format(std_err_i, '.1f')
    regression_line_dict_ref[name] = slope_dict_ref[name] * np.array(dates_dec_subset) + intercept_dict_ref[name]

#%%
fig=plt.figure(figsize=(14, 7))
# plot diff
for i, name in enumerate(dates_subset_ref_dict):
    dates_ref_plot = diff_cum_dict['Dates']
    diff_cum_plot = diff_cum_dict[name]
    plt.plot(dates_ref_plot, diff_cum_plot, label = 'Refd {}'.format(name))

plt.plot(merged_df['Dates'], differences_LOS[0:len(merged_df['Dates'])], label='Referenced GNSS {}'.format(gnss_station))
plt.plot(merged_df['Dates'], dfGPS_LOS_plot[0:len(merged_df['Dates'])], label='{}'.format(gnss_station))
plt.plot(merged_df['Dates'], dfGPS_ref_LOS[0:len(merged_df['Dates'])], label='Reference GNSS {}'.format(gnss_ref_station))
#plt.plot(merged_df['Dates'], regression_lineref)

#plt.plot(dfGPS['Dates'], dfGPS['dU'])

# Add labels and title to the plot
#plt.xlabel('Time')
plt.ylabel('LoS displacement (mm)')
plt.legend(fontsize=12)
plt.title('Referenced LoS time-series at point {}, {}'.format(gnss_lon, gnss_lat))

plt.text(0.5, -0.12, '{} referenced {} mm/yr'.format(gnss_station, slope_plotref), fontsize=10, ha='center', transform=plt.gca().transAxes)
plt.text(0.5, -0.16, '{} {} mm/yr'.format(gnss_station, slope_plot), fontsize=10, ha='center', transform=plt.gca().transAxes)

for i, name in enumerate(slope_plot_dict_ref):
    plt.text(0.5, -0.12-(i+2)*0.04, '{} referenced {} mm/yr'.format(name, slope_plot_dict_ref[name]), fontsize=10, ha='center', transform=plt.gca().transAxes)

for i, name in enumerate(slope_plot_dict):
    plt.text(0.5, -0.12-(i+4+2)*0.04, '{} {} mm/yr'.format(name, slope_plot_dict[name]), fontsize=10, ha='center', transform=plt.gca().transAxes)

plt.savefig('/nfs/see-fs-02_users/eejap/public_html/gnss_insar_outputs/{}/refd/referenced_time_series_{}_{}_{}_{}.jpg'.format(insar_frame, insar_frame,gnss_station, gnss_ref_station, ml_clip), dpi=400, bbox_inches='tight')
